...
if __name__ == "__main__":
    ...
    ###########################
    dataset_folder_path = os.path.join("datasets","coco")#"../datasets/coco"            #File path to dataset that you would like to extract objects from

    target_class_names = ["person", "motorcycle", "car", "airplane", "bus", "boat", "stop sign", "snowboard", "umbrella", "sports ball", "baseball bat", "bed", "tennis_racket", "suitcase", "skis"]                                   # Class names that you would like extracted from the dataset specified in dataset_folder_path

    translated_class_names = ["person", "motorcycle", "car", "airplane", "bus", "boat", "stop_sign", "snowboard", "umbrella", "sports_ball", "baseball_bat", "bed", "tennis_racket", "suitcase", "skis"]                                # Class names that correspond with the class names that you are extracting from the dataset. 
    ...
    yaml_data_path = glob.glob(os.path.join(dataset_folder_path,"*.yaml"))[0]     # File path to data.yaml file for dataset. This line will attempt to find it automatically
                                                                       # based on the provided dataset folder path.
    dataset_class_list_configuration = 1                               # Use 1 if data.yaml specifies names as a list. Use 0 if data.yaml specifies names with keys.
    ...
    label_files = glob.glob(os.path.join(dataset_folder_path,"labels", "**", "*.txt"), recursive=True)
    ...
    for annotation_file_path in label_files:
